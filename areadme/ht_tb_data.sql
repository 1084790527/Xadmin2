...
CREATE TABLE tb_account(
        ...
        sys_id VARCHAR(20),
        state VARCHAR(2) COMMENT '1启用 0停用 2删除',
        login_state VARCHAR(2) COMMENT '0 未登入  1 登入成功  2 登入失败  3 账号密码错误  4 掉线需要二次验证  5 掉线手动重新登入',
        login_id VARCHAR(50) COMMENT '淘宝登入账号',
        login_pass VARCHAR(50) COMMENT '淘宝登入密码',
        cre_oper_id VARCHAR(20)  COMMENT '创建人员id',
        cre_oper_date DATETIME COMMENT '添加日期',
        last_mod_oper_id VARCHAR(20) COMMENT '最后修改操作员id',
        last_mod_oper_date DATETIME COMMENT '最后修改操作员时间',
        PRIMARY KEY (id)
)
ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='淘宝账号设置';
ALTER TABLE tb_account ADD FULLTEXT (agency_id);
ALTER TABLE tb_account ADD FULLTEXT (sys_id);
ALTER TABLE tb_account ADD FULLTEXT (state);
ALTER TABLE tb_account ADD FULLTEXT (login_state);
ALTER TABLE tb_account ADD UNIQUE INDEX uniqueSysLoginId(sys_id, login_id) USING BTREE
