...
//    private TestService testService;
    ...
    @GetMapping(value = "/tbAccount/index")
    public String tbAccount(Model model){
        model.addAttribute("sysList",managerService.getAllSys());
        return "/tbAccount/index";
    }
                ...
                for(PrivilegesPojo two : one.getPrivileges()){
                    ...
                    if (two.getPrivileges() != null && two.getPrivileges().size()!= 0){
                        ...
                        for (PrivilegesPojo three : two.getPrivileges()){
                            ...
                            if (three.getPrivileges() != null && three.getPrivileges().size()!=0){
                                if (prIds.contains(three.getId()))
                                    threeData.put("spread",true);
                                JSONArray fourDataArray = new JSONArray();
                                for (PrivilegesPojo four : three.getPrivileges()){
                                    JSONObject fourData = new JSONObject();
                                    fourData.put("title", four.getName());
                                    fourData.put("id", four.getId());
                                    fourData.put("field","ids");
                                    if (prIds.contains(four.getId()))
                                        fourData.put("checked",true);
                                    fourDataArray.add(fourData);
                                }
                                threeData.put("children",fourDataArray);
                            }
                            if (prIds.contains(three.getId()))
