...
import com.yao.bean.vo.SelectVo;
...
public class ManagerService {
    ...
    public List<SelectVo> getAllSys() {
        LoginInfo info = (LoginInfo) session.getAttribute(Consts.LOGIN_INFO);
        if (!"1".equals(info.getDefaults()))
            return new ArrayList<>();
        List<SelectVo> select = new ArrayList<>();
        for (SysPojo sysPojo : sysDao.getRecordListByWhere(new SysPojo().setInState("1,0").setAgencyId(info.getAgencyId()))) {
            select.add(new SelectVo().setId(sysPojo.getId()).setValue(sysPojo.getNickname()));
        }
        return select;
    }
}
