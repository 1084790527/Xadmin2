...
public class generate {
    private static final String DATE_TABLE = "ht_tb_data";
    private static final String DATA_BASE = "tb_account";
    private static final String USER = "root";
    private static final String PASS = "wingzz00";
    private static final String JDBC_DRIVER = "com.mysql.cj.jdbc.Driver";
    private static final String DB_URL = "jdbc:mysql://localhost:3306/"+DATE_TABLE+"?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=UTC";
    public static void main(String[] args){
        ...
        try{
            ...
            sql = "Select COLUMN_NAME as name, DATA_TYPE as type, COLUMN_COMMENT as comment " +
                    ...
                    "Where table_name = '"+DATA_BASE+"' AND TABLE_SCHEMA = '"+DATE_TABLE+"'";
            ...
            stmt.close();
            conn.close();
            generatePojo(underline2Camel(DATA_BASE.replaceAll("tb_","")),results);
    ...
    public static void generatePojo(String className,List<ResultBean> results) throws IOException {
        ...
        for (ResultBean bean : results){
            String type = "";
            String jdbcType = "";
            if ("int".equals(bean.getType()) || "bigint".equals(bean.getType())){
                type = "Long";
                jdbcType = "NUMERIC";
                ...
                type = "Date";
                jdbcType = "TIMESTAMP";
            }else if ("decimal".equals(bean.getType())){
                type = "BigDecimal";
                jdbcType = "NUMERIC";
            }else{
                type = "String";
        ...
        //生成数据库实体类
        File pojo=new File("sys\\src\\main\\java\\test\\mysql\\"+className+"Pojo"+".java");
        if(pojo.exists()){       //如果存在这个文件就删除
            pojo.delete();
        }
        System.out.println(pojo.createNewFile());
        writeFileContent("sys\\src\\main\\java\\test\\mysql\\"+className+"Pojo"+".java", pojoContent);
        //生成mapper接口
        File dao=new File("sys\\src\\main\\java\\test\\mysql\\"+className+"Dao"+".java");
        if(dao.exists()){       //如果存在这个文件就删除
            dao.delete();
        }
        System.out.println(dao.createNewFile());
        writeFileContent("sys\\src\\main\\java\\test\\mysql\\"+className+"Dao"+".java", daoContent);
        //生成mapper数据库xml
        File mapper=new File("sys\\src\\main\\java\\test\\mysql\\"+className+"Mapper"+".xml");
        if(mapper.exists()){       //如果存在这个文件就删除
            mapper.delete();
        }
        System.out.println(mapper.createNewFile());
        writeFileContent("sys\\src\\main\\java\\test\\mysql\\"+className+"Mapper"+".xml", mapperContent);
    }
